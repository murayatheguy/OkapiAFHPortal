/**
 * Simple seed script to create 9 additional test Adult Family Homes
 * Each AFH has the same functionality as the existing Test Adult Family Home
 *
 * Login credentials:
 * - test@example.com / test123 (existing)
 * - test1@example.com through test9@example.com / test123 (new)
 */
...
async function seedTestAFHs() {
  console.log("🌱 Creating 9 additional test AFHs...\n");

  const hashedPassword = await bcrypt.hash(PASSWORD, 10);

  for (let i = 0; i < 9; i++) {
    const config = AFH_CONFIGS[i];
    const email = `test${i + 1}@example.com`;
    const ownerName = `${FIRST_NAMES[i]} ${LAST_NAMES[i]}`;

    console.log(`\n📍 Creating AFH ${i + 2}: ${config.name}`);
    console.log(`   Owner: ${ownerName} (${email})`);
    console.log(`   Location: ${config.city}, WA`);
    console.log(`   Specialty: ${config.specialty}`);

    // 1. Check/Create Owner
    ...
    if (!owner) {
      const [newOwner] = await db.insert(owners).values({
        ...
        name: ownerName,
        phone: `206-555-${String(1000 + i + 1).padStart(4, '0')}`,
        status: "active",
        emailVerified: true,
      }).returning();
      owner = newOwner;
      ...
      console.log(`   ⚠️  Owner exists`);
    }

    // 2. Check/Create Facility
    ...
    if (!facility) {
      const [newFacility] = await db.insert(facilities).values({
        ownerId: owner.id,
        name: config.name,
        slug: slugify(config.name) + '-' + randomBetween(100, 999),
        facilityType: 'afh',
        ...
        licenseNumber: `AFH-${String(200000 + (i + 1) * 1111)}`,
      ...
      console.log(`   ⚠️  Facility exists`);
    }

    // 3. Create Residents (2-5 per facility)
    ...
    if (existingResidents.length === 0) {
      const numResidents = randomBetween(2, Math.min(5, bedCount));
      ...
      console.log(`   ⚠️  Residents exist (${existingResidents.length})`);
    }

    // 4. Create Staff (2-4 per facility)
    ...
    if (existingStaff.length === 0) {
      ...
      for (let s = 0; s < numStaff; s++) {
        ...
        const staffEmail = `${staffFirst.toLowerCase()}.${staffLast.toLowerCase()}${i + 1}@carestaff.com`;
      ...
      console.log(`   ⚠️  Staff exist (${existingStaff.length})`);
    }

    // 5. Create/Update Facility Capabilities (for care matching)
    try {
      ...
      if (!existingCaps) {
        await db.insert(facilityCapabilities).values({
          facilityId: facility.id,
          careTypes: { afh: true, assistedLiving: false, skilledNursing: false, hospice: i === 4, respiteCare: i % 3 === 0, adultDaycare: false },
          specializations: {
            dementia: i === 0 || i === 5,
            alzheimers: i === 0,
            mentalHealth: i === 0,
          ...
          availability: {
            totalBeds: bedCount,
            currentOccupancy: existingResidents.length,
            availableBeds: bedCount - existingResidents.length,
            waitlistLength: 0,
  ...
  console.log("   test@example.com     (Sunrise AFH - existing)");
  for (let i = 0; i < 9; i++) {
    console.log(`   test${i + 1}@example.com     (${AFH_CONFIGS[i].name})`);
  }
  console.log("\n" + "═".repeat(50) + "\n");
}
